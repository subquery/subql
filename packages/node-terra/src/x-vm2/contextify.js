...
Decontextify.arguments = (args) => {
  ...
  try {
    ...
    for (let i = 0, l = args.length; i < l; i++)
      arr[i] = Decontextify.value(args[i]);
    return arr;
  } catch (e) {
    // Never pass the handled exception through!
...
Decontextify.instance = (instance, klass, deepTraps, flags, toStringTag) => {
  ...
  base.get = (target, key, receiver) => {
    ...
    if (key === '__defineGetter__')
      return host.Object.prototype.__defineGetter__;
    if (key === '__defineSetter__')
      return host.Object.prototype.__defineSetter__;
    if (key === '__lookupGetter__')
      return host.Object.prototype.__lookupGetter__;
    if (key === '__lookupSetter__')
      return host.Object.prototype.__lookupSetter__;
...
Decontextify.function = (fnc, traps, deepTraps, flags, mock) => {
  ...
  base.get = (target, key, receiver) => {
    try {
      ...
      if (key === 'isVMProxy') return true;
      if (mock && host.Object.prototype.hasOwnProperty.call(mock, key))
        return mock[key];
  ...
  base.defineProperty = (target, key, descriptor) => {
    ...
    if (descriptor.get || descriptor.set) {
      propertyDescriptor.get =
        Contextify.value(descriptor.get, null, deepTraps, flags) || undefined;
      propertyDescriptor.set =
        Contextify.value(descriptor.set, null, deepTraps, flags) || undefined;
      propertyDescriptor.enumerable = descriptor.enumerable === true;
      propertyDescriptor.configurable = descriptor.configurable === true;
    } else {
      propertyDescriptor.value = Contextify.value(
        descriptor.value,
        null,
        deepTraps,
        flags,
    ...
    if (!result) {
      try {
        if (local.Reflect.isExtensible(target)) {
          doPreventExtensions(target, object, (obj) =>
            Contextify.value(obj, null, deepTraps, flags),
          );
        }
      } catch (e) {
        // Should not happen
  ...
  if (host.Array.isArray(object)) {
    ...
    base.ownKeys = (target) => {
      try {
        ...
        return Decontextify.value(
          keys.filter((key) => typeof key !== 'string' || !key.match(/^\d+$/)),
        );
      } catch (e) {
        throw Decontextify.value(e);
      }
...
Contextify.instance = (instance, klass, deepTraps, flags, toStringTag) => {
  ...
  base.get = (target, key, receiver) => {
    ...
    if (key === '__defineGetter__')
      return local.Object.prototype.__defineGetter__;
    if (key === '__defineSetter__')
      return local.Object.prototype.__defineSetter__;
    if (key === '__lookupGetter__')
      return local.Object.prototype.__lookupGetter__;
    if (key === '__lookupSetter__')
      return local.Object.prototype.__lookupSetter__;
    if (key === host.Symbol.toStringTag && toStringTag) return toStringTag;

    try {
      return Contextify.value(
        host.Reflect.get(instance, key),
        null,
        deepTraps,
        flags,
      );
...
Contextify.object = (object, traps, deepTraps, flags, mock) => {
  ...
  base.defineProperty = (target, key, descriptor) => {
    ...
    if (descGet || descSet) {
      propertyDescriptor.get =
        Decontextify.value(descGet, null, deepTraps, flags) || undefined;
      propertyDescriptor.set =
        Decontextify.value(descSet, null, deepTraps, flags) || undefined;
      propertyDescriptor.enumerable = descriptor.enumerable === true;
      propertyDescriptor.configurable = descriptor.configurable === true;
    } else {
      propertyDescriptor.value = Decontextify.value(
        descValue,
        null,
        deepTraps,
        flags,
  ...
  base.isExtensible = (target) => {
    ...
    if (!result) {
      try {
        if (local.Reflect.isExtensible(target)) {
          doPreventExtensions(target, object, (obj) =>
            Decontextify.value(obj, null, deepTraps, flags),
          );
        }
      } catch (e) {
        // Should not happen
...
const LocalBuffer = (global.Buffer = Contextify.readonly(
  host.Buffer,
  BufferMock,
));
