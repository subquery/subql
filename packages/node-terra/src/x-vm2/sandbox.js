...
return ((vm, host) => {
  ...
  const EXTENSIONS = {
    ...
    ['.node'](module, filename) {
      if (vm.options.require.context === 'sandbox')
        throw new VMError(
          "Native modules can be required only with context set to 'host'.",
        );
    ...
    EXTENSIONS['.' + ext] = (module, filename, dirname) => {
      if (vm.options.require.context !== 'sandbox') {
        ...
        try {
          ...
          const code =
            host.STRICT_MODULE_PREFIX + contents + host.MODULE_SUFFIX;
  ...
  const _resolveFilename = (path) => {
    ...
    if (hasPackageJson) {
      ...
      try {
        ...
        throw new VMError(
          `Module '${path}' has invalid package.json`,
          'EMODULEINVALID',
        );
      }
    ...
    try {
      for (let i = 0; i < vm.options.sourceExtensions.length; i++) {
        const ext = vm.options.sourceExtensions[i];
        if (fs.existsSync(`${path}/index.${ext}`))
    ...
    /*
		if (moduleName === 'events' || moduleName === 'internal/errors') {
			...
			try {
				...
				//throw new VMError(`${BUILTIN_MODULES[moduleName]}`);
				throw new VMError(`${e}`);
			}
  ...
  const _prepareRequire = (currentDirname, parentAllowsTransitive = false) => {
    const _require = (moduleName) => {
      ...
      if (!requireObj)
        throw new VMError(
          `Access denied to require '${moduleName}'`,
          'EDENIED',
        );
      if (moduleName == null)
      ...
      if (BUILTIN_MODULES[moduleName]) {
        ...
        //throw new VMError(`${requireObj.builtin}`)
      ...
      if (/^(\.|\.\/|\.\.\/)/.exec(moduleName)) {
        ...
        if (typeof externalObj === 'object') {
          ...
          try {
            ...
            isWhitelisted =
              external.some((ext) => host.helpers.match(ext, moduleName)) ||
              (transitive && parentAllowsTransitive);
          } catch (e) {
            throw Contextify.value(e);
          }
          if (!isWhitelisted) {
            throw new VMError(
              `The module '${moduleName}' is not whitelisted in VM.`,
              'EDENIED',
            );
          }
        ...
        while (paths.length) {
          try {
            filename = _resolveFilename(requireObj.resolve(moduleName));
          } catch (e) {
            throw Contextify.value(e);
          }
          if (filename) break;
      ...
      try {
        ...
        if (rootObj) {
          const rootPaths = host.Array.isArray(rootObj)
            ? rootObj
            : host.Array.of(rootObj);
          allowedModule = rootPaths.some((path) =>
            host.String.prototype.startsWith.call(dirname, pa.resolve(path)),
          );
        }
      } catch (e) {
        throw Contextify.value(e);
      ...
      if (!allowedModule) {
        throw new VMError(
          `Module '${moduleName}' is not allowed to be required. The path is outside the border!`,
          'EDENIED',
        );
      }
      ...
      throw new VMError(
        `Failed to load '${moduleName}': Unknown type.`,
        'ELOADFAIL',
      );
    };
    ...
    listeners: function listeners(name) {
      ...
      try {
        return Contextify.value(
          host.process
            .listeners(name)
            .filter((listener) => Contextify.isVMProxy(listener)),
        );
      } catch (e) {
        throw Contextify.value(e);
      }
