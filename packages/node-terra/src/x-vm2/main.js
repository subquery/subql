...
const CACHE = {
  ...
  timeoutContext: null,
  timeoutScript: null,
  contextifyScript: loadAndCompileScript(
    `${__dirname}/contextify.js`,
...
class VMScript {
  ...
  constructor(code, options) {
    ...
    if (arguments.length === 2) {
      if (
        ...
        options.toString === Object.prototype.toString
      ) {
        useOptions = options || {};
  ...
  getCompiledCode() {
    if (!this._compiledCode) {
      this._compiledCode = this._compiler(
        this._prefix + removeShebang(this._code) + this._suffix,
        this.filename,
      );
    }
  ...
  _compileNodeVMStrict() {
    ...
    if (!script) {
      this._compiledNodeVMStrict = script = this._compile(
...
function makeCheckAsync(internal) {
  return (hook, args) => {
    if (
      ...
      const asyncMapped = args.map((arg) =>
        arg.replace(/async/g, 'a\\u0073ync'),
      );
      try {
...
class VM extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    if (hook) {
      if (!CACHE.hookScript) {
        CACHE.hookScript = loadAndCompileScript(
          `${__dirname}/fixasync.js`,
          '(function() { ',
          '\n})',
        );
        ...
        try {
          CACHE.getGeneratorFunctionScript = new vm.Script(
        ...
        try {
          CACHE.getAsyncFunctionScript = new vm.Script(
        ...
        try {
          CACHE.getAsyncGeneratorFunctionScript = new vm.Script(
            '(async function*(){}).constructor',
            {
      ...
      const internal = {
        __proto__: null,
        global: CACHE.getGlobalScript.runInContext(
          _context,
          DEFAULT_RUN_OPTIONS,
        ),
        internal: _internal,
      ...
      if (CACHE.getGeneratorFunctionScript) {
        try {
          internal.GeneratorFunction =
            CACHE.getGeneratorFunctionScript.runInContext(
              _context,
              DEFAULT_RUN_OPTIONS,
            );
        } catch (ex) {}
      ...
      if (CACHE.getAsyncFunctionScript) {
        try {
          internal.AsyncFunction = CACHE.getAsyncFunctionScript.runInContext(
            _context,
            DEFAULT_RUN_OPTIONS,
          );
        } catch (ex) {}
      ...
      if (CACHE.getAsyncGeneratorFunctionScript) {
        try {
          internal.AsyncGeneratorFunction =
            CACHE.getAsyncGeneratorFunctionScript.runInContext(
              _context,
              DEFAULT_RUN_OPTIONS,
            );
        } catch (ex) {}
    ...
    if (!this.timeout) {
      ...
      try {
        return this._internal.Decontextify.value(
          script.runInContext(this._context, DEFAULT_RUN_OPTIONS),
        );
      } catch (e) {
...
class NodeVM extends VM {
  ...
  constructor(options = {}) {
    ...
    const closure = sandboxScript.runInContext(
      this._context,
      DEFAULT_RUN_OPTIONS,
    );
  ...
  run(code, filename) {
    ...
    if (code instanceof VMScript) {
      script = this.options.strict
        ? code._compileNodeVMStrict()
        : code._compileNodeVM();
      ...
      script = new vm.Script(
        prefix + this._compiler(code, unresolvedFilename) + MODULE_SUFFIX,
        {
    ...
    try {
      ...
      return this._internal.Decontextify.value(
        wrapper === 'commonjs' ? module.exports : returned,
      );
    } catch (e) {
      throw this._internal.Decontextify.value(e);
    }
  ...
  static code(script, filename, options) {
    ...
    const resolvedFilename =
      typeof unresolvedFilename === 'string'
        ? pa.resolve(unresolvedFilename)
        : undefined;
