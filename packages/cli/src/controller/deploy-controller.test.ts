...
describe("splitMultichainDataFields test", () => {
  test("Single chain, single endpoint", () => {
    let result=splitMultichainDataFields("chainIdididid:http://1.1.1.1");
    expect(result).toEqual({ chainIdididid: 'http://1.1.1.1' }
    );
  });

  test("multi chain, multi endpoint", () => {
    let result=splitMultichainDataFields("chainIdididid:http://1.1.1.1,chainIdididid222:http://2.2.2.2,chainIdididid333:http://3.3.33.3");
  ...
  test("Incorrect values (endpoint without chainId)", () => {
    let result=splitMultichainDataFields("http://2.2.2.2,http://2.2.2.3");
