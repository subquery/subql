...
const MODEL_TEMPLATE_PATH = path.resolve(
  __dirname,
  '../terra/template/model.ts.ejs',
);
const MODELS_INDEX_TEMPLATE_PATH = path.resolve(
  __dirname,
  '../terra/template/models-index.ts.ejs',
);
const TYPES_INDEX_TEMPLATE_PATH = path.resolve(
  __dirname,
  '../terra/template/types-index.ts.ejs',
);
const INTERFACE_TEMPLATE_PATH = path.resolve(
  __dirname,
  '../terra/template/interface.ts.ejs',
);
const ENUM_TEMPLATE_PATH = path.resolve(
  __dirname,
  '../terra/template/enum.ts.ejs',
);
...
export async function generateJsonInterfaces(
  projectPath: string,
  schema: string,
): Promise<void> {
  ...
  if (jsonInterfaces.length !== 0) {
    ...
    try {
      await renderTemplate(
        ...
        path.join(typesDir, `interfaces.ts`),
        interfaceTemplate,
      );
...
export async function codegen(projectPath: string): Promise<void> {
  ...
  const manifest = loadTerraProjectManifest(projectPath);
  await generateJsonInterfaces(
    projectPath,
    path.join(projectPath, manifest.schema.file),
  );
  ...
  if (exportTypes.interfaces || exportTypes.models || exportTypes.enums) {
    try {
      await renderTemplate(
        TYPES_INDEX_TEMPLATE_PATH,
        path.join(projectPath, TYPE_ROOT_DIR, `index.ts`),
        {
          props: {
            exportTypes,
          },
...
export async function generateModels(
  ...
  for (const entity of extractEntities.models) {
    ...
    const indexedFields = fields.filter(
      (field) => field.indexed && !field.isJsonInterface,
    );
    const modelTemplate = {
      props: {
        baseFolderPath,
  ...
  if (classNames.length !== 0) {
    try {
      await renderTemplate(
        MODELS_INDEX_TEMPLATE_PATH,
        path.join(projectPath, MODEL_ROOT_DIR, `index.ts`),
        {
          props: {
            classNames,
          },
