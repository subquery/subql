...
if (GeneratorFunction) {
  ...
  override(
    ...
    'constructor',
    new host.Proxy(GeneratorFunction, makeCheckFunction('generator_function')),
  );
}
if (AsyncFunction) {
  ...
  override(
    ...
    'constructor',
    new host.Proxy(AsyncFunction, makeCheckFunction('async_function')),
  );
}
if (AsyncGeneratorFunction) {
  ...
  override(
    ...
    new host.Proxy(
      AsyncGeneratorFunction,
      makeCheckFunction('async_generator_function'),
    ),
  );
}
...
if (Promise) {
  Promise.prototype.then = new host.Proxy(
    Promise.prototype.then,
    makeCheckFunction('promise_then'),
  );
  Contextify.connect(host.Promise.prototype.then, Promise.prototype.then);

  if (Promise.prototype.finally) {
    Promise.prototype.finally = new host.Proxy(
      Promise.prototype.finally,
      makeCheckFunction('promise_finally'),
    );
    Contextify.connect(
      host.Promise.prototype.finally,
      Promise.prototype.finally,
    );
  }
  if (Promise.prototype.catch) {
    Promise.prototype.catch = new host.Proxy(
      Promise.prototype.catch,
      makeCheckFunction('promise_catch'),
    );
