...
Файл `schema.graphql` определяет различные схемы GraphQL. Из-за того, как работает язык запросов GraphQL, файл схемы по существу определяет форму ваших данных из SubQuery. Чтобы узнать больше о том, как писать в GraphQL-языке мы рекомендуем проверить [Schemas and Type](https://graphql.org/learn/schema/#type-language).

**Важно: когда вы вносите какие-либо изменения в файл схемы, убедитесь, что вы повторно создали каталог типов с помощью следующей команды ` yarn codegen `**

### Сущности
Каждая сущность должна определять свои обязательные поля ` id ` с типом ` ID! `. Она используется в качестве первичного ключа и уникальна между всеми сущностями одного типа.
